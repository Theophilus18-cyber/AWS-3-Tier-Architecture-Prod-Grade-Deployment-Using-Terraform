# Development Environment - Vault Server Setup
# This creates a Vault server running in DEV MODE
...
resource "aws_security_group" "vault_dev" {
  name        = "vault-dev-sg"
  description = "Security group for Vault dev server"
  vpc_id      = var.vpc_id
...
resource "aws_instance" "vault_dev" {
  ...
  vpc_security_group_ids = [aws_security_group.vault_dev.id]
  ...
  tags = {
    ...
    Environment = "dev"
    ManagedBy   = "Terraform"
    Purpose     = "HashiCorp Vault Development Server"
  }
  ...
  root_block_device {
    ...
    encrypted   = true
    tags = {
      Name = "vault-dev-root-volume"
    }
  }
}

# Elastic IP for Vault Server (optional but recommended)
resource "aws_eip" "vault_dev" {
  instance = aws_instance.vault_dev.id
  domain   = "vpc"
